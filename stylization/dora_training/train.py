            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    if mask_latents is not None and mask_latents.shape[1] == x.shape[1]:
                        mask = mask_latents.squeeze(-1).bool()  # [1, N] -> bool mask
                        update = torch.zeros_like(result)

                        masked_input = x[mask]  # [N_masked, D]
                        masked_output = lora_B(lora_A(masked_input))  # [N_masked, D]
                        update[mask] = masked_output * scaling

                        result = result + update
                    else:
                        update = lora_B(lora_A(x)) * scaling
                        result = result + update
                else:
                    ...
                    if mask_latents is not None and mask_latents.shape[1] == x.shape[1]:
                        ...
                        masked_input = x[mask]  # [N_masked, D]
                        masked_output = self.lora_magnitude_vector[active_adapter](
                            masked_input,
                            lora_A=lora_A,
                            lora_B=lora_B,
                            scaling=scaling,
                            base_layer=self.get_base_layer(),
                            base_result=base_result[mask] if base_result is not None else None,
                        ).to(torch.float16)
                        update[mask] = masked_output
                    else:
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="mask",
        help="The name of the training dataset (from the HuggingFace hub) to train on.",
    )
    parser.add_argument(
        "--validation_prompt",
        type=str,
...
def main(args):
    ...
    # print("column_names: ", column_names)  # ['image', 'text', 'mask]
    ...
    if args.mask_column is None:
        mask_column = dataset_columns[2] if dataset_columns is not None else column_names[2]
    else:
    ...
    def preprocess_train(examples):
        masks = [
            Image.open(os.path.join(args.train_data_dir, fname)).convert("L")
            for fname in examples[mask_column]
        ]
        ...
        for idx, (image, mask) in enumerate(zip(images, masks)):
            ...
            mask = np.array(mask)
            mask = (mask > 128).astype(np.float32)  # Binarization
        ...
        examples["mask_pixel_values"] = all_masks
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.pretrained_vae_model_name_or_path is not None:
                    ...
                    mask_values = batch["mask_pixel_values"].to(dtype=weight_dtype)
                else:
                    pixel_values = batch["pixel_values"]
                    mask_values = batch["mask_pixel_values"]
                ...
                # Resize mask
                mask_latents = F.interpolate(mask_values,
                                            size=(model_input.shape[-2], model_input.shape[-1]),
                                            mode="nearest")
                # print("model_input.shape", model_input.shape)  # [1, 4, 128, 128]
                # print("mask_values.shape", mask_values.shape)  # [1, 1, 128, 128]
                ...
                # print("noisy_model_input.shape", noisy_model_input.shape)  # [1, 4, 128, 128]
                # print("mask_values.shape", mask_values.shape)  # [1, 1, 128, 128]
                model_pred = unet(
                    noisy_model_input,
                    timesteps,
                ...
                if noise_scheduler.config.prediction_type == "epsilon":  # execute this
                    target = noise
                ...
                if args.snr_gamma is None:  # execute this
                    # loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")

                    # only compute loss on masked region
                    mask = mask_latents.expand_as(model_pred)  # [B, 4, H, W]
                    loss = F.mse_loss(
                        model_pred.float() * mask,
                        target.float() * mask,
                        reduction="sum"
                    ) / mask.sum().clamp(min=1.0)  # 防止除以 0
                else:
