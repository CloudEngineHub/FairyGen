...
class Transformer2DModel(LegacyModelMixin, LegacyConfigMixin):
        ...
        **kwargs,
        ...
        for block in self.transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                # print(kwargs)  # {'mask_latents': tensor..., device='cuda:0', dtype=torch.float16)}
                # print(self.__class__)  # transformer_2d.Transformer2DModel

                def _gradient_checkpointing_func(module, *args, **kwargs):  # modify
                ...
                # print(block)  # BasicTransformerBlock
                hidden_states = block(
                    hidden_states,
    ...
    def _operate_on_continuous_inputs(self, hidden_states):
        ...
        if not self.use_linear_projection:
            ...
            inner_dim = hidden_states.shape[1]
            hidden_states = hidden_states.permute(0, 2, 3, 1).reshape(batch, height * width, inner_dim)
            # print(self.proj_in.__class__.__name__)  # Linear
            hidden_states = self.proj_in(hidden_states)
