...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if input_ndim == 4:
            batch_size, channel, height, width = hidden_states.shape
            hidden_states = hidden_states.view(batch_size, channel, height * width).transpose(1, 2)  # [B, C, HxW] -> [B, HxW, C]
        ...
        if mask_latents is not None:
            ...
            # Downsample mask_latents to (spatial x spatial)
            mask_resized = F.interpolate(mask_latents, size=(spatial, spatial), mode="nearest")

            # Reshape to [B, N, 1]
            mask_latents = mask_resized.view(B, 1, spatial * spatial).transpose(1, 2)  # [B, N, 1]

        # print(mask_latents.shape)  # [1, 4096, 1]
        batch_size, sequence_length, _ = (
            hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
        )
        ...
        # query = attn.to_q(hidden_states) 
        query = attn.to_q(hidden_states, mask_latents=mask_latents) 
        ...
        # key = attn.to_k(encoder_hidden_states)
        # value = attn.to_v(encoder_hidden_states)
        key = attn.to_k(encoder_hidden_states, mask_latents=mask_latents)
        value = attn.to_v(encoder_hidden_states, mask_latents=mask_latents)
        ...
        # linear proj
        # hidden_states = attn.to_out[0](hidden_states)
        hidden_states = attn.to_out[0](hidden_states, mask_latents=mask_latents)
        # dropout
        hidden_states = attn.to_out[1](hidden_states)
