...
#             # print("mask_latents.shape", mask_latents.shape)  # [2, 1, 128, 128]
#             # print("hidden_states.shape", hidden_states.shape)  # [2, 4096, 640], [2, 1024, 1280]
#             B, N, C = hidden_states.shape
#             spatial = int(N ** 0.5)
#             assert spatial * spatial == N, f"Unexpected token count {N}, not a square map"

#             mask_resized = F.interpolate(mask_latents, size=(spatial, spatial), mode="nearest")  # downsample
#             mask_latents = mask_resized.view(B, 1, spatial * spatial).transpose(1, 2)  # [B, N, 1]

#             # print(mask_latents.shape)  # [2, 4096, 1], [2, 1024, 1]
        ...
        if mask_latents is not None:
            query = attn.to_q(hidden_states, *args, mask_latents=mask_latents)
        else:
            query = attn.to_q(hidden_states, *args)
        ...
        if mask_latents is not None:
            key = attn.to_k(encoder_hidden_states, *args, mask_latents=mask_latents)
            value = attn.to_v(encoder_hidden_states, *args, mask_latents=mask_latents)
        else:
            key = attn.to_k(encoder_hidden_states, *args)
            value = attn.to_v(encoder_hidden_states, *args)
        ...
        if mask_latents is not None:
            hidden_states = attn.to_out[0](hidden_states, *args, mask_latents=mask_latents)
        else:
            hidden_states = attn.to_out[0](hidden_states, *args)
