...
class UNetMidBlock2D(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, temb: Optional[torch.FloatTensor] = None, **kwargs) -> torch.FloatTensor:
        hidden_states = self.resnets[0](hidden_states, temb)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                hidden_states = attn(hidden_states, temb=temb, **kwargs)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i, (resnet, attn) in enumerate(blocks):
            # print(f"Layer {i} attention type: {type(attn)}")
            if self.training and self.gradient_checkpointing:
