...
class StableDiffusionXLBrushNetPipeline(
    ...
    def __call__(
        ...
        # 4. Prepare image
        if isinstance(brushnet, BrushNetModel):
            image = self.prepare_image(  # to tensor
                image=image,
                width=width,
                height=height,
            ...
            # print(image.dtype)
            original_mask = (original_mask.sum(1)[:, None, :, :] < 0).to(image.dtype)  # bool, [B, 1, H, W]
        ...
        # print(type(image), image.shape if hasattr(image, 'shape') else 'no shape')
        # print(type(self.vae))
        ...
        # 7.1 Create tensor stating which brushnets to keep (timesteps)
        brushnet_keep = []
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print(mask.shape)  # [2, 1, 128, 128]
                # mask_latents = mask  # TODO: mask:1-M, 1-mask:M
